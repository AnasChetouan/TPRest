...
namespace ClientTrivago
{
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            List<Offre> offresIbis = new List<Offre>();
            List<Offre> offresBooking = new List<Offre>();
            Dictionary<int, string> imagesChambres = new Dictionary<int, string>();

            HttpResponseMessage responseBooking = await clientBooking.GetAsync(path);
            HttpResponseMessage responseIbis = await clientIbis.GetAsync(path);
            if (responseBooking.IsSuccessStatusCode && responseIbis.IsSuccessStatusCode)
            {
                offresIbis = await responseBooking.Content.ReadAsAsync<List<Offre>>();
                offresBooking = await responseBooking.Content.ReadAsAsync<List<Offre>>();
            }
            else
            {
                offresIbis = null;
            ...
            List<List<Offre>> grandeOffre = new List<List<Offre>>();
            grandeOffre.Add(offresIbis);
            grandeOffre.Add(offresBooking);

            // Télécharger l'image d'une chambre d'après son ID :

            // https://localhost:44348/api/Chambres/1/GetImage

            foreach (Offre o in offresBooking) {
                ...
                responseBooking = await clientIbis.GetAsync(path);
            ...
            foreach (Offre o in offresIbis)
            {
                int idChambre = o.IdChambre;
                path = "api/Chambres/" + idChambre + "/GetImage";
                //client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue(System.Net.Mime.MediaTypeNames.Application.Octet));// header
                responseIbis = await clientIbis.GetAsync(path);

                if (responseIbis.IsSuccessStatusCode)
                {
                    //System.Net.Mime.MediaTypeNames.Application.Octet
                    // var image = await response.Content.ReadAsAsync<FileResult>();
                    byte[] imageBytes = await clientIbis.GetByteArrayAsync(path);
            ...
            Offres offresForm = new Offres(offresBooking, imagesChambres, dateArr, dateDep, nbClients);
            Offres offresForms = new Offres(offresIbis, imagesChambres, dateArr, dateDep, nbClients);
            offresForm.Tag = this;
            offresForm.Show(this);
            offresForms.Tag = this;
            offresForms.Show(this);
            Hide();
